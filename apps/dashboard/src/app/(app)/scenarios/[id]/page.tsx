...
function persistSteps(id: string, steps: FlowStep[]) {
  try {
    ...
    console.log('[SimForge] Saved', steps.length, 'steps for', id);
  } catch (e) {
...
function tokenizeLine(line: string): React.ReactNode[] {
  ...
  while (rem.length > 0) {
    ...
    if (fm) {
      ...
      tokens.push(
        <span key={i++} className={`${c} rounded-sm px-0.5 font-semibold`}>
          {fm[1]}
        </span>,
      );
      rem = rem.slice(fm[1].length);
      continue;
    }
    ...
    if (sm) {
      ...
      if (raw.includes('{{')) {
        ...
        parts.forEach((p) => {
          if (p.startsWith('{{'))
            tokens.push(
              <span key={i++} className={`${getFakerColor(p)} rounded-sm px-0.5 font-semibold`}>
                {p}
              </span>,
            );
          else
    ...
    if (sp) {
      tokens.push(<span key={i++}>{sp[1]}</span>);
      rem = rem.slice(sp[1].length);
      continue;
    }
    tokens.push(
...
const [steps, setSteps] = useState<FlowStep[]>(() => {
  const loaded = loadSteps(id);
  console.log('[SimForge] Loaded', loaded.length, 'steps from storage');
  return loaded;
});
...
useEffect(() => {
  ...
  if (stored !== current) {
    persistSteps(id, steps);
    console.log('[SimForge] Persisted', steps.length, 'steps');
  }
}, [steps, id]);
  ...
  async function handleRun() {
    try {
      ...
      steps.forEach((step, i) => {
        ...
        const allHeaders = {
          ...Object.fromEntries(
            step.headers.filter((h) => h.key && h.value).map((h) => [h.key, h.value]),
          ),
          ...authHeaders,
        };
        ...
        nodes[nodeId] = {
          ...
          action: {
            ...
            headers: allHeaders,
            bodyTemplate:
              step.body && step.body.trim() !== '{}' && step.body.trim() !== ''
                ? step.body.trim()
                : null,
          },
      ...
      nodes['done'] = {
        ...
        transitions: [],
        cooldownMs: 0,
        thinkTimeMs: { meanMs: 0, stdDevMs: 0 },
        maxRetries: 0,
      };
              ...
              <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                <div className="flex items-center gap-2 px-3 py-1.5 border-b border-zinc-800/30 flex-shrink-0 bg-[#111]">
                      ...
                      <div className="absolute top-full left-0 mt-1 w-64 bg-[#1c1c1c] border border-zinc-700 rounded-lg shadow-2xl z-50 overflow-hidden">
                        <div className="px-3 py-2 border-b border-zinc-800 bg-[#161616]">
                          ...
                          <p className="text-[10px] text-zinc-700 mt-0.5">
                            Click to insert · or type your own
                          </p>
                        </div>
                  ...
                  <span className="text-[10px] text-zinc-700 ml-auto">
                    JSON · faker templates supported
                  </span>
                </div>
                ...
                {activeStep.headers.map((header, i) => (
                  <div key={i} className="flex items-center gap-1.5">
                    <input
                      value={header.key}
                      onChange={(e) => {
                        const h = [...activeStep.headers];
                        h[i] = { ...h[i], key: e.target.value };
                        updateStep(activeStep.id, { headers: h });
                      }}
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div>
                  ...
                  <p className="text-[11px] text-zinc-600 mb-4 leading-relaxed">
                    Capture values and use in later steps via{' '}
                    <span className="text-yellow-400 font-mono">
                    ...
                    {activeStep.extractRules.map((rule, i) => (
                      <div key={i} className="flex items-center gap-1.5">
                        <input
                          value={rule.varName}
                          onChange={(e) => {
                            const r = [...activeStep.extractRules];
                            r[i] = { ...r[i], varName: e.target.value };
                            updateStep(activeStep.id, { extractRules: r });
                          }}
                          placeholder="userId"

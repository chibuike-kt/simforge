...
const FAKER_VARS = [
  ...
  { group: 'Phone', items: [
    { label: 'Phone Number',      value: '{{faker.phone.number}}' },
    { label: 'Nigeria (any)',     value: '{{faker.phone.nigeria}}' },
    { label: 'Nigeria (MTN)',     value: '{{faker.phone.nigeria.mtn}}' },
    { label: 'Nigeria (Airtel)',  value: '{{faker.phone.nigeria.airtel}}' },
    { label: 'Nigeria (Glo)',     value: '{{faker.phone.nigeria.glo}}' },
...
function storageKey(id: string) { return `sf_flow_${id}`; }
...
function loadSteps(id: string): FlowStep[] {
  try {
    ...
    if (raw) {
      const parsed = JSON.parse(raw) as FlowStep[];
      if (Array.isArray(parsed) && parsed.length > 0) return parsed;
    }
  } catch (e) {
    console.warn('[SimForge] Failed to load steps:', e);
  }
  return [defaultStep()];
}
...
function persistSteps(id: string, steps: FlowStep[]) {
  try {
    ...
    window.localStorage.setItem(storageKey(id), JSON.stringify(steps));
  } catch (e) {
    console.warn('[SimForge] Failed to save steps:', e);
  }
}
...
function getFieldName(variable: string): string {
  ...
  const map: Record<string, string> = {
    ...
    'faker.internet.username.safe': 'username',
    ...
    'faker.finance.pin': 'pin', 'faker.finance.pin.4': 'pin',
    'faker.finance.pin.6': 'passcode', 'faker.finance.pin.8': 'passcode',
    'faker.location.city': 'city', 'faker.location.country': 'country',
    'faker.location.streetAddress': 'address', 'faker.location.zipCode': 'zipCode',
    'faker.commerce.productName': 'product', 'faker.commerce.price': 'price',
    'faker.company.name': 'company', 'faker.date.future': 'expiresAt',
    'faker.date.past': 'createdAt', 'faker.date.birthdate': 'dateOfBirth',
    'faker.number.int': 'count', 'faker.number.float': 'value',
    'faker.number.digits.4': 'pin', 'faker.number.digits.6': 'passcode',
    'faker.number.digits.8': 'passcode',
    'faker.phone.nigeria': 'phone', 'faker.phone.nigeria.mtn': 'phone',
    'faker.phone.nigeria.airtel': 'phone', 'faker.phone.nigeria.glo': 'phone',
    'faker.phone.nigeria.9mobile': 'phone',
...
export default function ScenarioPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  console.log('[ScenarioPage] id:', id, typeof id);
  ...
  const { data: runsData } = useScenarioRuns(id);
  ...
  useEffect(() => {
    ...
    if (stored !== current) persistSteps(id, steps);
  }, [steps, id]);
  ...
  async function handleRun() {
    try {
      ...
      steps.forEach((step, i) => {
        ...
        nodes[nodeId] = {
          id: nodeId, type: 'http', label: step.name,
          action: {
            method: step.method,
      ...
      await submitRun.mutateAsync({
        id, agentCount: scale,
        flowSteps: nodes, entryNodeId: 'step_0',
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-[#0f0f0f]">
      ...
      <div className="flex items-center gap-3 px-3 py-2 border-b border-zinc-800/60 bg-[#161616] flex-shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            ...
            History
            {runs.length > 0 && (
              <span className="text-[10px] text-zinc-500 tabular-nums">{runs.length}</span>
            )}
          </button>
      ...
      {/* ── RUN HISTORY PANEL ───────────────────────────────── */}
      {showHistory && (
        <div className="border-b border-zinc-800/60 bg-[#141414] flex-shrink-0">
          <div className="px-4 py-2 border-b border-zinc-800/40 flex items-center gap-2">
            <History size={11} className="text-zinc-600" />
            <span className="text-[10px] font-medium text-zinc-600 uppercase tracking-wider">Run History</span>
            <span className="text-[10px] text-zinc-700">{runs.length} run{runs.length !== 1 ? 's' : ''}</span>
          </div>
          {runs.length === 0 ? (
            <div className="flex items-center justify-center py-6">
              <p className="text-xs text-zinc-700">No runs yet — hit Run to start your first simulation</p>
            </div>
          ) : (
            <div className="flex gap-0 overflow-x-auto max-h-40">
              {[...runs].reverse().map((run, i) => {
                const status = run.status as string;
                const dot = status === 'completed' ? 'bg-green-400'
                  : status === 'dispatched' || status === 'running' ? 'bg-blue-400 animate-pulse'
                  : status === 'failed' ? 'bg-red-400' : 'bg-zinc-600';
                const statusColor = status === 'completed' ? 'text-green-400'
                  : status === 'dispatched' ? 'text-blue-400'
                  : status === 'failed' ? 'text-red-400' : 'text-zinc-500';
                return (
                  <div key={run.id as string}
                    className="flex-shrink-0 w-48 border-r border-zinc-800/40 p-3 hover:bg-zinc-800/30 transition-colors">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <div className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0', dot)} />
                      <span className={cn('text-[10px] font-medium uppercase tracking-wide', statusColor)}>
                        {status}
                      </span>
                    ...
                    <p className="text-[10px] font-mono text-zinc-600 truncate mb-1">
                      {(run.id as string).slice(0, 20)}...
                    </p>
                    <p className="text-[10px] text-zinc-600">
                      {run.createdAt
                        ? new Date(run.createdAt as string).toLocaleString('en-US', {
                            month: 'short', day: 'numeric',
                            hour: '2-digit', minute: '2-digit',
                          })
                        : '—'}
                    </p>
                    {run.scenarioVersion !== undefined && (
                      <p className="text-[10px] text-zinc-700 mt-0.5">v{run.scenarioVersion as number}</p>
                    )}
                  </div>
                );
              })}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-52 flex-shrink-0 border-r border-zinc-800/60 flex flex-col overflow-hidden bg-[#141414]">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <button onClick={addStep}
              ...
              <Plus size={10} /><span className="text-[11px]">Add step</span>
            </button>
          </div>
        </div>
        ...
        <div className="w-[420px] flex-shrink-0 flex flex-col border-r border-zinc-800/60 overflow-hidden bg-[#0f0f0f]">
          ...
          <div className="flex items-center px-3 py-2.5 border-b border-zinc-800/60 gap-0 flex-shrink-0">
            ...
            <input value={activeStep.url} onChange={e => updateStep(activeStep.id, { url: e.target.value })}
          ...
          <div className="px-3 py-1.5 border-b border-zinc-800/30 flex-shrink-0">
            <input value={activeStep.name} onChange={e => updateStep(activeStep.id, { name: e.target.value })}
              ...
              <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                ...
                <div className="flex-1 relative overflow-hidden bg-[#0d0d0d] min-h-0">
                  <textarea value={activeStep.body}
                    ...
                    spellCheck={false} style={{ caretColor: '#d4d4d8' }} />
